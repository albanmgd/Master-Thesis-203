...
# Calculate realized volatility using a rolling window of 30 days
realized_volatility = log_returns.rolling(window=22).std().dropna()

# Calculate realized volatility at month t+1 as a function of the realized volatility at month t
vol_t = realized_volatility[:-1]
vol_t1 = realized_volatility[1:]
...
ax.scatter(vol_t, vol_t1, alpha=0.5)
ax.plot([np.min(vol_t), np.max(vol_t)], [np.min(vol_t), np.max(vol_t)], color="green")
ax.plot(vol_t, slope*vol_t + intercept, color="green", label=f"R²={r_squared:.2f}")
